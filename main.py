...
# --- ХЕНДЛЕР ДЛЯ ПРИЛОЖЕНИЯ ---
@dp.message(F.content_type == "web_app_data")
async def handle_web_app_data(message: types.Message):
    data = json.loads(message.web_app_data.data)
    if "location" in data:
        lat, lon = data["location"]["lat"], data["location"]["lon"]
        loc = geolocator.reverse((lat, lon), language='ru')
        await process_ai_logic(message, f"Я здесь: {loc.address}. Расскажи всё про это место.")
    elif "action" in data:
        if data["action"] == "guide": await back_to_guide(message)
        elif data["action"] == "translator": await show_main_translator_menu(message)

# --- ВСЕ ВАШИ ПРЕЖНИЕ ХЕНДЛЕРЫ ---
@dp.message(F.text == "🌐 Переводчик")
async def show_main_translator_menu(message: types.Message):
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="👤 Я", callback_data="side_user"), InlineKeyboardButton(text="🤖 Анжела", callback_data="side_bot")]])
...
async def start(message: types.Message):
    init_db()
    await react(message, "Привет! Я Анжела. Я твой универсальный гид и переводчик. Чем могу помочь?", sticker_key="greeting")
